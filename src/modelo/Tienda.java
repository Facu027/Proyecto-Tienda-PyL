...
public class Tienda {
    ...
    /*
     ...
     Se optó por utilizar la estructura Map<modelo.Usuario, Double> (y Map<modelo.Usuario, Integer>) por las siguientes razones:
        ...
        El Map permite unificar y acumular el historial de cada cliente asociando un objeto único (modelo.Usuario) a un valor totalizador.
...
/* Retorna el cliente que más plata gastó en el total de todas sus compras
    public modelo.Usuario obtenerClienteMasComprador() {
        ...
        Map<modelo.Usuario, Double> gastoPorCliente = new HashMap<>();
        for (modelo.Carrito venta : ventasRealizadas) {
            modelo.Usuario cliente = venta.getCliente();
            double totalVenta = venta.calcularTotal();
        ...
        modelo.Usuario clienteTop = null;
        double maxGasto = -1;
        for (Map.Entry<modelo.Usuario, Double> clienteGasto : gastoPorCliente.entrySet()) {
            if (clienteGasto.getValue() > maxGasto) {
                clienteTop = clienteGasto.getKey(); //Clave, objeto modelo.Usuario
